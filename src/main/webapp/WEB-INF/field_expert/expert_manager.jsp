...
<button type="button" class="btn btn-primary" style="margin-left:10px;" onclick="window.open('${APP_PATH}/goto_expert_content_manager/edit_expert_by_id/0/.do')"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br><br>
<br style="clear:both;">
	...
	<script src="${APP_PATH}/static/js/bootstrap/js/select2.js"></script>
        ...
        $("#send_excel").click(function(){
            ...
            function pageQuery( pageIndex ) {
            	...
            	$.ajax({
            		url : "${APP_PATH}/goto_expert_content_manager/expert/pageQuery.do",
            		type : "POST",
            		data : dataObj,
            		beforeSend : function() {
            				...
            				$.each(pageObj.datas, function(index, expertDto){
	            	            ...
	            	            expert += '<tr>';
	          	                ...
	          	                expert += '  <td style="text-align:center;">';
	          	              	expert += '      <a type="button" title="编辑" onclick="window.open(\'${APP_PATH}/goto_expert_content_manager/edit_expert_by_id/'+expertDto.id+'/'+$("#sel_menu1").select2().val()+'.do\')" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></a>';	
            ...
            function deleteexpert(id) {
    			layer.confirm("删除专家，是否继续",  {icon: 3, title:'提示'}, function(cindex){
    				...
    				$.ajax({
    					url : "${APP_PATH}/goto_expert_content_manager/delete_expert_by_id.do",
    					type : "POST",
    					data : {
    						id : id
    					},

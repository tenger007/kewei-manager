...
public interface MenuManagerServiceInf {
	//根据菜单名称和技术端id查询菜单id
  ...
  //根据第三方id和技术点名称查找对应的菜单
    List<Long> query_user_menuid_by_userid_jsdid(String userid,String jsdid);
  ...
  //根据技术点名称查找菜单个数
    ...
    //根据多个菜单id删除菜单
     int delete_menusDto_ids(Datas ids );
    
    //删除技术点下所有的菜单
    int delete_menu_jsdid(Long jsdid);
    
  //查询结果分页
  	Page<MenuDto> queryMenuPage(Map<String, Object> paramMap);
  	List<MenuDto> queryMenus4Page(Integer pageno, Integer pagesize);
  	int queryUserCount4Page();
}
